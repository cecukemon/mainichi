...
- **Voice selection.** *Resolved (live run, 2026-07-19):* the shipped pair `ja-JP-Neural2-B` / `ja-JP-Neural2-C` sounds right for beginner-paced Japanese — no change needed. Voice ids remain constants in one place (`tts_service.dart`) since they participate in the audio cache key; swapping one later self-invalidates the cache.
- **Gaps between chained line files.** *Resolved (live run, 2026-07-19):* the natural inter-file gap reads as a comfortable dialogue pause on device — no explicit gap insertion or SSML `<break>` machinery needed.
