...
- Picture-derived words: do we retain/display the cropped drawing region in the review card, or just a text note that it was picture-derived? Depends on whether the extractor currently returns image regions.
- **Extractor needs to return kanji candidates, not just one guess** — the kanji-chip design (§3) assumes the model can offer ranked alternates plus a bounding region for the photo crop. `worksheet_extractor.dart` currently returns a single kanji value per word; needs checking/extending to emit alternates + region, or the review card degrades to "confirm the one reading, or pick no kanji" with no real alternates.
- Visual design: triage screen, and three review-queue item variants (vocab with kanji-candidates, template, dedup-merge) are mocked up in Claude Design. Picture-derived word still uses the plain vocab card layout — no distinct mockup yet.

---

## 5. Status

`[in progress]` — all 5 screens implemented in Flutter (`lib/capture/`), driven by a hand-written fixture draft rather than a live photo/API call (deferred, see §4). Riverpod (`CaptureQueueNotifier`) manages queue navigation and draft edits; `commit_service.dart` writes approved items into the real drift schema, including the same-batch and cross-import dedup paths. Covered by unit tests (`test/capture/dedup_test.dart`, `test/capture/commit_service_test.dart`) and an end-to-end widget test (`test/capture/capture_flow_test.dart`).

Still open before this is real: wiring `image_picker`/camera capture, a `dio`-based Anthropic API client to call the extractor live, and the extractor changes noted in §4 (kanji candidates, image crop regions). The done-screen "Review" action on an already-committed skipped item is a read-only stub — reopening a skip post-commit needs its own design pass.
