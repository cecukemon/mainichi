...
- [done] reading exercise screen — implemented per `features/reading-exercise.md` and the D43 mockups (D47, 2026-07-07): continuous feed, script layout (speaker name in a left margin column), furigana toggle, loading/error states with explicit retry, word-tap lookup sheet (reading/meaning/role chip/D44 form annotation/dictionary form for conjugated surfaces), punctuation rendered from `text` per D42 (`lib/reading/line_display.dart`), out-of-scope generations discarded into the error state, home entry replacing the deleted spike screen. Widget-tested end to end against fakes; *live simulator run still pending* (first in-app generation call + furigana visual check + latency measurement)
