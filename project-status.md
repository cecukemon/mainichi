...
- [planned] reading exercise screen — flow and key UX decided (`features/reading-exercise.md`, decision log D39–D42): continuous feed of generated conversations, hands-off on topic, no self-rating step, script-style layout (speaker name in a left margin column) with a word-tap lookup sheet (reading/meaning/role chip/dictionary form for conjugated surfaces) as the only interactivity, furigana toggle carried over from the spike. Visual design now done — Claude Design handoff received and matches, saved as 6 screens in `features/` (decision log D43). Not yet implemented; blocked on a live `GenerationSeed` from the real store (repository/query layer, below) and on deciding where the lookup sheet's per-token grammatical-form annotation comes from (`features/reading-exercise.md` §4)
